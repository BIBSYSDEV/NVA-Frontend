...
describe('Search', () => {
  ...
  it('The user should see a working pagination', () => {
    ...
    cy.get(`[data-testid=${dataTestId.startPage.searchPagination}] button`).eq(6).should('be.enabled');
    cy.get(`[data-testid=${dataTestId.startPage.searchPagination}] button`).eq(2).click();
    cy.get(`[data-testid=${dataTestId.startPage.searchPagination}] button`).eq(0).should('be.enabled');
