...
import { ImportCandidateSearchResponse } from '../types/importCandidate.types';
...
export const fetchImportCandidates = async (results: number, from: number, query = '') => {
  ...
  const getImportCandidates = await authenticatedApiRequest2<ImportCandidateSearchResponse>({
